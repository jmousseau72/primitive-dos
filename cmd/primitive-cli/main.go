// Command primitive-cli is the original fogleman/primitive command-line
// interface, preserved flag-for-flag so scripted workflows keep working.
// Differences from upstream: animated GIF output is encoded natively (no
// ImageMagick required), and the deprecated rand.Seed call is gone (Go 1.20+
// seeds automatically).
package main
			...
			for _, output := range Outputs {
				...
				if saveFrames || last {
					...
					case ".gif":
						...
						check(session.SaveAnimatedGIF(path, frames, 50, 250, 0))
					}
				}
			}
		}
